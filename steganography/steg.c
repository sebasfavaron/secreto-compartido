...
uint8_t* decode(uint8_t* input, int secret_length, bool LSB2){
    ...
    uint8_t *in_pixelarray = (uint8_t *)calloc(1, in_size);
...
void run_paper(int argc , char *argv[]) {
    ...
    uint8_t *secret_stream = (uint8_t *)calloc(1, s_size);
    ...
    for(int i=0; i<amount_partitions; i++){
        ...
        uint8_t *partition = decode(outputs[i], partition_size, LSB2);
